...
export default function ahsiyetlerAdminPage() {
  const isKavram = false as boolean;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="font-serif text-display-sm text-primary-text mb-2">
            Şahsiyetler Yönetimi
          </h1>
          <p className="text-secondary-text text-sm">
            Tüm şahsiyetler taslaklarını ve yayınlanmış içerikleri yönetin.
          </p>
        </motion.div>
        
        <Link
          href="/admin/sahsiyetler/yeni"
              ...
              {data.slice(0, 15).map((item, idx) => (
                <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link
                      href={`/admin/sahsiyetler/${item.slug}`}
